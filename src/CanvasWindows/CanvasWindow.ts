...
abstract class CanvasWindow extends Box {
    ...
    layer: number = 0;
    _camera: CanvasWindow | undefined;
    ...
    connectDisplay(camera: CanvasWindow, canvas: HTMLCanvasElement) {
        this._camera = camera;
        this._canvas = canvas;
    }
    ...
    prepareHandlerProps() : HandlerProps {
        ...
        const camera = this._camera as CanvasWindow;
    ...
    onMouseDownPrimitive(pos: Vec2,terminateEvent: () => void) {
        if (!this.onMouseDown) return;
        this.onMouseDown(this.prepareMouseHandlerProps(pos,terminateEvent));
    }
    onMouseDown?(p: MouseHandlerProps) {};


    onMouseMovePrimitive(pos: Vec2,terminateEvent: () => void) {
        if (!this.onMouseMove) return;
        this.onMouseMove(this.prepareMouseHandlerProps(pos,terminateEvent));
    }
    onMouseMove(p: MouseHandlerProps) {};
    
    onMouseUpPrimitive(pos: Vec2,terminateEvent: () => void) {
        if (!this.onMouseUp) return;
        this.onMouseUp(this.prepareMouseHandlerProps(pos,terminateEvent));
    }
    onMouseUp(p: MouseHandlerProps) {};
    
    onClickPrimitive(pos: Vec2,terminateEvent: () => void) {
        if (!this.onClick) return;
        this.onClick(this.prepareMouseHandlerProps(pos,terminateEvent));
    }
    onClick(p: MouseHandlerProps) {};

    update(p : any) {};
}

export type { HandlerProps, RenderProps, MouseHandlerProps };
export { CanvasWindow };